...
- Run the `/review` **loop** on every PR before declaring it ready to merge. It is not a single pass: each round runs a cold reviewer with no conversation context plus 2 independent skeptics per finding (a finding is **confirmed** only if it survives adversarial verification); every confirmed finding gets fixed and pinned with a test that fails on revert; rounds repeat until one returns zero confirmed (cap 4). The in-session review runs alongside — it knows the intent, which the cold agents don't — and its findings go through the same skeptics before they count. Fixes introduce bugs (PR #888: rounds 2–4 each caught defects from the previous round's fixes), which is why it loops.
- **Confirmed → fix, any tier, without asking.** "Not sure" is a to-do, not a label — dig until it's confirmed or refuted. Only a genuine product-behavior question ("is this the *intended* behavior?") goes to the user, in plain English with the evidence of what was tried. This applies in `/review`, `/autopilot`, and the autonomous loop alike; `/autopilot` runs the loop on every PR it pushes before reporting done.
